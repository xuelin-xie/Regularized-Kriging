function  [dmodel, perf] = epetdacefit(S, Y, regr, corr, theta0, lob, upb, alpha, gamma)
...
mzmax = m*(m-1) / 2;        % 点组合的总数
ij = zeros(mzmax, 2);       % initialize matrix with indices y值的组合
...
par = struct('corr',corr, 'regr',regr, 'y',Y, 'F',F, ...
  'D', D, 'ij',ij, 'scS',sS,'alpha',alpha,'gamma',gamma);
...
dmodel = struct('regr',regr, 'corr',corr, 'theta',theta.', ...
  ...
  'C',fit.C, 'Ft',fit.Ft, 'G',fit.G, 'Y',yyy);
...
detR = prod( full(diag(C)) .^ (2/m) );  %|R|.^1/m
obj = sum(sigma2) * detR + par.gamma*sum(par.alpha*abs(theta.^(1))+(1-par.alpha)*theta.^(2));
if  nargout > 1
